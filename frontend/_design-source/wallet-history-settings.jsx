...
const {
  ...
  FeatherWatermark, PoolCapacityBar,
} = window;
...
function WalletScreen({ pushToast, tweaks }) {
  const totalCapacity = POOL_CHILDREN.length * 18 - POOL_CHILDREN.reduce((s, c) => s + c.pending, 0);
  const [showMaster, setShowMaster] = useStateW(false);
  return (
    <div style={{ padding: 24, height: "100%", overflow: "auto" }} className="q-scroll">
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        {/* Pool exhaustion banner (conditional — show if all near cap) */}
        {tweaks.poolExhausted && (
          <div style={{
            ...
            <i data-lucide="alert-octagon" style={{ width: 18, height: 18, color: "var(--aztec-vermillion)", strokeWidth: 1.5 }}></i>
            <div style={{ flex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))", gap: 16 }}>
          {POOL_CHILDREN.map(c => (
            <ChildCard key={c.id} child={c} onFaucet={() => pushToast({ tone: "success", title: `Faucet sent to child-${c.id}`, detail: "0.1 ETH fee-juice", tx: "0xdrip…" })} />
          ))}
        </div>
        ...
        <div className="q-card-deep q-card" style={{ padding: 24 }}>
          ...
          <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", background: "rgba(0,0,0,0.3)", borderRadius: 6 }}>
            <i data-lucide="key" style={{ width: 16, height: 16, color: "var(--aztec-chartreuse)", strokeWidth: 1.5 }}></i>
            <code style={{
...
function ChildCard({ child, onFaucet }) {
  return (
...
function BalanceRow2({ token, kind, amount, label }) {
  return (
...
function HistoryScreen({ tweaks }) {
  const [side, setSide] = useStateW("all");
  const [hideDecoys, setHideDecoys] = useStateW(false);
...
function SettingsScreen() {
  const [network, setNetwork] = useStateW("alpha-testnet");
  return (
    <div style={{ padding: 24, height: "100%", overflow: "auto" }} className="q-scroll">
...
function Toggle({ defaultOn = false }) {
  const [on, setOn] = useStateW(defaultOn);
  return (
...
Object.assign(window, { WalletScreen, HistoryScreen, SettingsScreen });
if (window.qStatus) qStatus("Loaded · wallet/history/settings");

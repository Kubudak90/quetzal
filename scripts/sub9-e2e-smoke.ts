...
async function main(): Promise<void> {
  ...
  if (state.step < 5) {
    ...
    // escrow leg → requires PRIVATE balance. The wizard frontend / SDK has no
    // public→private conversion helper, so we add one here. After
    // canonicalization on Sub-9's testnet addrs, side="buy"
    // tUSDC->tETH flips to side="sell" tETH->tUSDC → input token (escrow) is
    // tUSDC (path[path_len-1] when realSide=true). So move some public tUSDC

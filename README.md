...
**Sub-2.5 LANDED:** end-to-end clearing is live. The circuit's `fn main` emits 42 public fields matching the orderbook's `flatten_clearing_public`; the aggregator's `traceBucketSwap` is a true multi-bucket state machine; the witness builder + bb prove pipeline are reconfirmed (500-field proof, 115-field VK, EMPTY_ROOT unchanged).
